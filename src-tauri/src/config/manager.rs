...
use super::{config::{ConfigFile, ConfigType}, save_data_config};
...
impl ConfigManager {
    pub fn new() -> Self {
        ...
        let settings_path = "data/settings.toml";
        let save_data_path = "data/save_data.toml";

        let settings = SettingsConfig::load_or_create(settings_path)
        .expect("Failed to load settings");
    let save_data = SaveDataConfig::load_or_create(save_data_path)
        ...
        Self {
            ...
            save_data_path: save_data_path.to_string(), 
        }
...
#[tauri::command]
pub fn set_settings_config(config_manager: State<'_, Arc<Mutex<ConfigManager>>>, db_path: String, num: u32) {
    let mut config_manager = config_manager.lock().map_err(|e| e.to_string()).expect("Failed to lock config_manager");
...
#[tauri::command]
pub fn set_save_data_config(config_manager: State<'_, Arc<Mutex<ConfigManager>>>, test_val: String) {
    let mut config_manager = config_manager.lock().map_err(|e| e.to_string()).expect("Failed to lock config_manager");
...
pub fn save_configs(config_manager: State<'_, Arc<Mutex<ConfigManager>>>) {
    let config_manager = config_manager.lock().map_err(|e| e.to_string()).expect("Failed to lock config_manager");
